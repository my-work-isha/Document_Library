...
import { SwTransaction } from './sw-transaction.model';
...
@Component({
  ...
  templateUrl: './sw-transaction.component.html',
  styleUrls: ['./sw-transaction.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class SwTransactionComponent implements OnInit {
  ...
  isReadOnly: boolean;
  userLoggedIn: string;
  isLoading: boolean = true;    //sw-trans is read-only before fields are retrieved completely
          ...
          if (this.task === "create") {
            ...
            if (mbpModel) {
              if (this.model._id == "") {
                ...
                this.model.txTransactionNum = "";
                this.model.txEnduserTr = "";
                // this.model.txTransactionNum = mbpModel.txTransactionNum;
                // this.model.txEnduserTr = mbpModel.txEnduserTr;
            ...
            this.isLoading = false;   //sw-transaction becomes editable after the fields are retrieved completely
            ...
            attch_params.append("form", "frmSW");
            ...
            this.http.get('/api/getAttachedFileListFromDB2', attach_options).subscribe(
              ...
              () => {
                _.forEach(this.attachmentList, function (row) {
                  ...
                  $("#" + fieldId + "File").append("<div><a id='att_" + fileId + "' class='file' href='/api/getFileFromDB2/frmSW/" + fileID + "/" + fileName.replace("%", "%25") + "'>" + fileName + "</a><a id='del_" + fileId + "'class='delete' onclick='deleteFile(\"frmSW\",\"" + fileID + "\",\"" + fileName.replace("%", "%25") + "\",\"" + deleteFiles + "\")'> x</a></div>");
                })
    ...
    this.viewdetails.getDocbyid(params).subscribe(
      ...
      () => {
        if (this.dbRevID != this.model._rev && this.model._id != "") {
          ...
          this.http.post('/api/insertswtransaction',this.model, options).subscribe(
            data => {
  ...
  hasFileSizeExceeded(): boolean {
    ...
    if (this.sizeFiles1 + this.sizeFiles2 + this.sizeFiles3 + this.sizeFiles4 +
      ...
      this.sizeFiles9 + this.sizeFiles10 > 0) {
  ...
  copyDocLink() {
    ...
    selBox.value = applurl + "swtransactiondoc/" + this.model._id;
